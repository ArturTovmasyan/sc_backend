...
class CustomerInvalidRentCommand extends Command
{
    use LockableTrait;

    protected static $defaultName = 'app:customer:invalid-rent';
    ...
    {
        $this
            ->setHelp('Notify admins if invalid resident rent for all customers command.');
    }
        ...
        try {
            ...
            foreach ($vhosts as $vhost) {
                if ($vhost->getCustomer()->isEnableLedgerCommands()) {
                    ...
                    $output->writeln(sprintf("Notify admins if invalid resident rent for '%s'...", $domain));

                    $this->createEnv($db, $mailer, $dir_name, $domain);
                    $this->customerRentIncrease($dir_name['root']);

                    $output->writeln(sprintf("Completed invalid resident rent notification for '%s'.", $domain));
                }
            }
        } catch (\Throwable $e) {
            dump($e->getMessage());
    ...
    private function customerRentIncrease($root_dir)
    {
        ...
        $process = new Process(
            [$path['php'], $path['symfony_console'], 'app:invalid-resident-rent'],
