...
class CustomerDashboardCommand extends Command
{
    use LockableTrait;
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        try {
            ...
            foreach ($vhosts as $vhost) {
                ...
                $output->writeln(sprintf("Dashboard for '%s'...", $domain));

                $this->createEnv($db, $mailer, $dir_name, $domain);
                $this->customerDashboard($dir_name['root']);

                $output->writeln(sprintf("Completed dashboard for '%s'.", $domain));
            }
        } catch (\Throwable $e) {
            dump($e->getMessage());
    ...
    private function customerDashboard($root_dir)
    {
        ...
        $process = new Process(
            [$path['php'], $path['symfony_console'], 'app:facility:dashboard'],
