...
class CustomerWebEmailsGrabberCommand extends Command
{
    use LockableTrait;

    protected static $defaultName = 'app:customer:webemailgrabber';
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        try {
            ...
            foreach ($vhosts as $vhost) {
                ...
                $output->writeln(sprintf("WebEmailsGrabber for '%s'...", $domain));

                $this->createEnv($db, $mailer, $dir_name, $domain);
                $this->customerDashboard($dir_name['root'], $domain);

                $output->writeln(sprintf("Completed WebEmailsGrabber for '%s'.", $domain));
            }
        } catch (\Throwable $e) {
            dump($e->getMessage());
    ...
    private function customerDashboard($root_dir, $domain)
    {
        ...
        $process = new Process(
            [$path['php'], $path['symfony_console'], 'app:webemailgrabber', $domain],
